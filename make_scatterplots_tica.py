...
def plot_scatterplot(tica_embed, topk, words, left_axis_index, length, output_path):
    n, dim = tica_embed.shape
    ...
    for axis_idx in axis_idxs:
        top_word_ids = np.argsort(normed_tica_embed[:, axis_idx])[-k:]
    ...
    picked_emb = normed_tica_embed[:, axis_idxs]  # (n, length)
    ...
    for ax_idx in range(2):
        ...
        if ax_idx == 1:
            ...
            picked_skews = np.mean(tica_embed[:, axis_idxs] ** 3, axis=0)
        ...
        print(f"c_I: {calc_c_I(picked_emb, normed_tica_embed, topk): .2f}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scatterplot for TICA.")

    parser.add_argument("--emb_type", type=str, default="glove")
    parser.add_argument("--width", type=int, default=9)
    parser.add_argument("--left_axis_index", type=int, default=50)
...
def main():
    ...
    output_dir = Path("output/images/scatterplots_tica")
    output_dir.mkdir(exist_ok=True, parents=True)
    output_path = (
        output_dir / f"scatterplot_{emb_type}_width{width}_"
        f"left{left_axis_index}_length{length}.png"
    )

    plot_scatterplot(tica_embed, topk, words, left_axis_index, length, output_path)

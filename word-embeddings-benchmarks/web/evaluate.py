...
def evaluate_categorization(w, X, y, method="all", seed=None):
    ...
    if method == "all" or method == "kmeans":
        purity = calculate_purity(
            y[ids],
            KMeans(random_state=seed, n_init=10, n_clusters=len(set(y))).fit_predict(
                words[ids]
            ),
        )
...
def evaluate_on_semeval_2012_2(w):
    ...
    for c in categories:
        ...
        prot_left = np.mean(
            np.vstack(w.get(word, mean_vector) for word in prototypes[:, 0]), axis=0
        )
        prot_right = np.mean(
            np.vstack(w.get(word, mean_vector) for word in prototypes[:, 1]), axis=0
        )
...
def evaluate_on_all(w):
    ...
    logger.info("computing similarity benchmarks")
    similarity_tasks = {
        "MEN": fetch_MEN(),
        "WS353": fetch_WS353(),
    ...
    logger.info("computing analogy benchmarks")
    ...
    for name, data in iteritems(analogy_tasks):
        ...
        logger.info(
            "Analogy prediction accuracy on {} {}".format(name, analogy_results[name])
        )
    ...
    logger.info("computing categorization benchmarks")
